...
Instance modifiers based on intended players, if a 5 man dung with a solo player
scale difficulty by x'ing player stats by 5 same logic applies to 25, 40 whatever
...
public:
    ...
    void OnLogin(Player* Player, bool /*firstLogin*/) override
    {
        ...
        if (SoloCraftEnable)
        {
            ...
            {
                ChatHandler(Player->GetSession()).SendSysMessage("|cff4CFF00SoloCraft Balance |r is running.");
            }
        }
    }
};
    ...
    void ApplyBuffs(Player* player, Map* map, float difficulty, int dunLevel, int numInGroup)
    {
        ...
        if (difficulty != 0)
        {
            ...
            else
            {
                ...
                ss << "|cffFF0000[SoloCraft] |cffFF8000" << player->GetName() << " entered %s  - |cffFF0000You have not been buffed. |cffFF8000 Your level is higher than the max level (%i) threshold for this dungeon. 25man(%b), dun(%b), diffi(%f)";
                ChatHandler(player->GetSession()).PSendSysMessage(ss.str().c_str(), map->GetMapName(), dunLevel + SolocraftLevelDiff, map->Is25ManRaid(), map->IsDungeon(), difficulty);
            }
        }
        else
